...
- [ ] **`/api/health` route** — JSON `{ ok, version, ts, sha }` ping
      for uptime monitors (BetterStack, Updown, internal). Cheap +
      universally expected.
- [ ] **`/api/v1/openapi.json`** — full OpenAPI 3.1 spec describing
      every endpoint we ship. Generated by hand into a typed
      object today; eventually replaceable with auto-gen from
      Zod schemas.
